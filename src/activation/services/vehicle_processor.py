...
class VehicleProcessor:
    def __init__(
        ...
        bmw_api: callable,
        hm_api: callable,
        stellantis_api: callable,
        renault_api: callable,
        volkswagen_api: callable,
        kia_api: callable,
        tesla_api: callable,
        df: pd.DataFrame,
    ):
    ...
    async def _get_oem(self, cursor, oem_raw: str) -> str:
        ...
        cursor.execute("SELECT id FROM oem WHERE LOWER(oem_name) = %s", (oem_lower,))
        result = cursor.fetchone()
        return result[0]

    async def _get_or_create_make(self, cursor, con, make_raw: str, oem_id: str) -> str:
        """Get or create Make record."""
        make_lower = MAKE_MAPPING.get(make_raw, make_raw.lower())

        cursor.execute(
            "SELECT id FROM make WHERE LOWER(make_name) = %(make_name)s",
            {"make_name": make_lower},
        )
        result = cursor.fetchone()
        ...
        if not result:
            ...
            cursor.execute(
                """
                INSERT INTO make (id, make_name, oem_id)
                VALUES (%(id)s, %(make_name)s, %(oem_id)s)
                RETURNING id
                """,
                {"id": make_id, "make_name": make_lower, "oem_id": oem_id},
            )
            con.commit()
            return cursor.fetchone()[0]
        return result[0]

    async def _get_company_id(self, cursor, con, company: str) -> str | None:
        """Get COMPANY ID by fleet name."""
        cursor.execute(
            "SELECT id FROM company WHERE LOWER(name) = LOWER(%(company)s)",
            {"company": company},
        )
        result = cursor.fetchone()
        ...
        if result:
            ...
            cursor.execute(
                ...
                RETURNING id
                """,
                {"name": company, "description": "", "id": company_id},
            )
            con.commit()
            return cursor.fetchone()[0]
        else:
    ...
    async def _get_fleet_id(
        self, cursor, con, fleet: str, company: str = ""
    ) -> str | None:
        """Get Fleet ID by fleet name."""
        cursor.execute(
            "SELECT id FROM fleet WHERE LOWER(fleet_name) = LOWER(%(fleet)s)",
            {"fleet": fleet},
        )
        result = cursor.fetchone()
        ...
        if result:
            ...
            company_id = await self._get_company_id(cursor, con, company)
            fleet_id = str(uuid.uuid4())

            cursor.execute(
                """
                INSERT INTO fleet (fleet_name, company_id, id)
                VALUES (%(fleet_name)s, %(company_id)s, %(id)s)
                RETURNING id
                """,
                {"fleet_name": fleet, "company_id": company_id, "id": fleet_id},
            )
            con.commit()
            return cursor.fetchone()[0]
        else:
    ...
    async def _get_or_create_region(self, cursor, con, country: str) -> str:
        """Get or create Region record."""
        country = COUNTRY_MAPPING.get(country, country)

        cursor.execute(
            "SELECT id FROM region WHERE LOWER(region_name) = LOWER(%(region)s)",
            {"region": country},
        )
        result = cursor.fetchone()
        ...
        if not result:
            ...
            cursor.execute(
                ...
                VALUES (%(id)s, %(region_name)s)
                RETURNING id
                """,
                {"id": region_id, "region_name": country},
            )
            con.commit()
            return cursor.fetchone()[0]
        return result[0]
    ...
    async def _get_or_create_renault_model(
        self,
        ...
        cursor.execute(
            "SELECT id, autonomy FROM vehicle_model WHERE LOWER(model_name) = %s AND LOWER(type) = %s AND LOWER(version) = %s",
        ...
        if model_exists:
            if autonomy is None:
                ...
                cursor.execute(
                    "UPDATE vehicle_model SET autonomy = %s WHERE id = %s",
                    (wltp_range, model_exists),
                )
                logging.info(
                    f"Updated model with name {model_name} and type {model_type} and version {version} and autonomy {wltp_range}"
                )
            return model_exists
        else:
            model_id = str(uuid.uuid4())
            ...
            logging.info(
                f"Created new model with name {model_name} and type {model_type} and version {version} and autonomy {wltp_range}"
            )
        return model_id
    ...
    async def _get_or_create_other_model(
        ...
        """Get a model if it exists then update it, or create it if it doesn't exist."""
        cursor.execute(
            "SELECT id FROM vehicle_model WHERE LOWER(model_name) = %s AND LOWER(type) = %s AND LOWER(version) = %s",
            (model_name.lower(), model_type.lower(), version.lower()),
        )
        result = cursor.fetchone()
        model_exists = result[0] if result else None
                ...
                with get_connection() as con:
                    ...
                    for _, vehicle in renault_df.iterrows():
                        try:
                            ...
                            cursor.execute(
                                "SELECT id FROM vehicle WHERE vin = %s", (vin,)
                            )
                            vehicle_exists = cursor.fetchone()
                            vehicle_exists = (
                                vehicle_exists[0] if vehicle_exists else None
                            )
                            (
                                model_name,
                                model_type,
                            ...
                            logging.error(
                                f"Error processing Renault vehicle {vehicle['vin']}: {e!s}"
                            )
                            con.rollback()
                            continue
        except Exception as e:
            logging.error(f"Error in Renault processing: {e!s}")
            raise
                ...
                with get_connection() as con:
                    ...
                    for _, vehicle in bmw_df.iterrows():
                        try:
                            ...
                            logging.error(
                                f"Error processing BMW vehicle {vehicle['vin']}: {e!s}"
                            )
                            con.rollback()
                            continue
        except Exception as e:
            logging.error(f"Error in BMW processing: {e!s}")
            raise
        ...
        try:
            ...
            with get_connection() as con:
                ...
                for _, vehicle in other_df.iterrows():
                    try:
                        cursor.execute(
                            "SELECT id FROM vehicle WHERE vin = %s", (vehicle["vin"],)
                        )
                        vehicle_exists = cursor.fetchone()
                        fleet_id = await self._get_fleet_id(
                            cursor, con, vehicle["fleet"], vehicle["company"]
                        ...
                        if not vehicle_exists:
                            ...
                            cursor.execute(
                                ...
                                (
                                    vehicle_id,
                                    await self._clean_value(vehicle["vin"]),
                                    fleet_id,
                                    region_id,
                                    model_id,
                            ...
                            logging.info(
                                f"New vehicle inserted in DB VIN: {vehicle['vin']}"
                            )
                        else:
                            cursor.execute(
                        ...
                        logging.error(
                            f"Error processing vehicle {vehicle['vin']}: {e!s}"
                        )
                        con.rollback()
                        continue
        except Exception as e:
            logging.error(f"Error in other vehicles processing: {e!s}")
            raise
    ...
    async def process_deactivated_vehicles(self) -> None:
        ...
        try:
            ...
            logging.error(f"Error in deactivated vehicles processing: {e!s}")
            if "con" in locals():
                con.rollback()
            raise
    ...
    async def delete_unused_models(self) -> None:
        ...
        try:
            with get_connection() as con:
                ...
                cursor.execute(
                    "SELECT COUNT(*) FROM vehicle_model WHERE id NOT IN (SELECT vehicle_model_id FROM vehicle)"
                )
                count = cursor.fetchone()[0]

                # Delete the unused models
                cursor.execute(
                    "DELETE FROM vehicle_model WHERE id NOT IN (SELECT vehicle_model_id FROM vehicle)"
                )
                con.commit()

                logging.info(f"Deleted {count} unused vehicle models from the database")
        except Exception as e:
            logging.error(f"Error in deleting unused models: {e!s}")
            raise
    ...
    async def generate_vehicle_summary(self) -> None:
        ...
        try:
            with get_connection() as con:
                ...
                print(
                    f"{'Total Vehicles:':<30} {active_vehicles + deactivated_vehicles:>10}"
                )
            ...
            logging.error(f"Error generating vehicle summary: {e!s}")
            raise
                ...
                with get_connection() as con:
                    ...
                    for _, vehicle in tesla_df.iterrows():
                        try:
                            ...
                            # Check if vehicle exists
                            cursor.execute(
                                "SELECT id, vehicle_model_id FROM vehicle WHERE vin = %s",
                                (vin,),
                            )
                            result = cursor.fetchone()
                            ...
                            if not result:
                                ...
                                logging.info(
                                    f"New Tesla vehicle inserted in DB VIN: {vin}"
                                )
                            else:
                                ...
                                cursor.execute(
                                    "SELECT version FROM vehicle_model WHERE id = %s",
                                    (model_id,),
                                )
                                ...
                                if not current_version:
                                    ...
                                    if api_version != "MTU":
                                        ...
                                        cursor.execute(
                                            "UPDATE vehicle SET vehicle_model_id = %s WHERE id = %s",
                                            (new_model_id, vehicle_id),
                                        )
                                else:
                                    (
                                        warranty_km,
                            ...
                            logging.error(
                                f"Error processing Tesla vehicle {vehicle['vin']}: {e!s}"
                            )
                            con.rollback()
                            continue
        except Exception as e:
            logging.error(f"Error in Tesla processing: {e!s}")
            raise
