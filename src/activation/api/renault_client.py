...
class RenaultApi:
    def __init__(self, kid: str, aud: str, client: str, scope: str, pwd: str):
        ...
        if not os.path.exists(self.pkcs12_file):
            raise FileNotFoundError(
                f"Required file {self.pkcs12_file} not found in current directory"
            )
    ...
    async def _generate_token(self) -> dict:
        ...
        try:
            ...
            self._run_openssl_command(
                f"openssl base64 -d -a -in {self.pkcs12_file} -out keystore.p12"
            )
            self._run_openssl_command(
                f"openssl pkcs12 -in keystore.p12 -clcerts -nodes -nocerts -passin pass:{self.pwd} -out private.key"
            )
            ...
            # Step 5: Sign the JWT
            self._run_openssl_command(
                f"openssl dgst -sha256 -sign private.key -out jwt_signature.bin {unsigned_file}"
            )
            ...
            while retry_count < max_retries:
                try:
                    ...
                    async with session.get(url, headers=headers) as response:
                        ...
                        error_message = RenaultAPIErrorCode.get_error_message(
                            error_code
                        )
                        ...
                        if error_code in [401, 429, 500, 503]:
                            ...
                            if error_code == 429:
                                ...
                                print(
                                    f"Retryable error occurred. Waiting {wait_time} seconds before retry..."
                                )
                        ...
                        if error_code in [400, 403, 404]:
                            raise RenaultAPIError(
                                f"{error_message} (HTTP {error_code})"
                            )

                        raise RenaultAPIError(
                            f"Unexpected error: {error_message} (HTTP {error_code})"
                        )
                    ...
                    if retry_count < max_retries - 1:
                        ...
                        wait_time = min(2**retry_count, 60)
                        print(
                            f"Network error occurred. Waiting {wait_time} seconds before retry..."
                        )
                        await asyncio.sleep(wait_time)
                        continue
                    raise RenaultAPIError(
    ...
    async def get_vehicle_wltp_range(
        self, session: aiohttp.ClientSession, vin: str
    ) -> int:
        """
