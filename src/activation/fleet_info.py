...
async def read_fleet_info(fleet_filter: str | None = None) -> pd.DataFrame:
    ...
    if fleet_col is None:
        raise ValueError(
            f"Could not find fleet column. Available columns: {df.columns.tolist()}"
        )
    ...
    if fleet_filter:
        ...
        logger.info(
            f"Found {len(df)} vehicles for fleet {fleet_filter} (filtered from {initial_count})"
        )
    ...
    df[["oem", "make", "model", "type"]] = (
        df[["oem", "make", "model", "type"]].astype(str).apply(lambda x: x.str.lower())
    )
