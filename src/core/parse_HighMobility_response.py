...
When called as a script, it takes in two mandatory path arguments: `json_source`, `csv_dest`.
"""
...
def main():
    ...
    df = parse_json_obj(kwargs["json_source"])
    df.to_csv(read_json(kwargs["csv_dest"]))

def parse_json_obj(src) -> DF:
...
def flatten_json_obj(src:dict, dst:dict[Any,dict[str,Any]]={}, timestamp=None, path:list[str]=[]) -> dict[Any,dict[str,Any]]:
    if "timestamp" in src:
        timestamp = DT.strptime(src["timestamp"], "%Y-%m-%dT%H:%M:%S.%fZ")

    try_add_to_dst(src, dst, timestamp, path, "data")
    try_add_to_dst(src, dst, timestamp, path, "value")
