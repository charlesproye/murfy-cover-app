...
def filter_results_by_lines_bounds(
    results: DF, valid_soh_points: DF, logger: Logger = logger
) -> DF:
    """
    ### Description:
    ...
    if results["soh"].isna().all():
        ...
        return results
    max_intercept, max_slope = intercept_and_slope_from_points(
        valid_soh_points.xs("max", level=0, drop_level=True)
    )
    min_intercept, min_slope = intercept_and_slope_from_points(
        valid_soh_points.xs("min", level=0, drop_level=True)
    )
    results = (
        results.eval("raw_soh = soh")
        .eval(f"max_valid_soh = odometer * {max_slope:f} + {max_intercept:f}")
        .eval(f"min_valid_soh = odometer * {min_slope:f} + {min_intercept:f}")
        .eval(
            f"soh_is_valid = soh.between(min_valid_soh, max_valid_soh) & soh.between(0.5, 1.0)"
        )
    ...
    if results.shape[0]:
        ...
        if nb_rows_removed == results.shape[0]:
            logger.warning(
                f"While filtering, all SoH results were set to NaN, check the valid SOH points."
            )
        else:
            logger.debug(
                f"Filtered SoH results out of bounds defined in processed_results.config, {nb_rows_removed}({rows_removed_pct:.2f}%) set to NaN."
            )
    else:
...
def intercept_and_slope_from_points(points: DF) -> tuple[float, float]:
    slope = (points.at["B", "soh"] - points.at["A", "soh"]) / (
        points.at["B", "odometer"] - points.at["A", "odometer"]
    )
...
def lr_params_as_series(df: DF, x: str, y: str) -> Series:
    ...
    if df.empty or df[x].eq(df[x].iat[0]).all():
        return Series(
            data=[np.nan] * 6,
            index=["slope", "intercept", "rvalue", "pvalue", "stderr", "r2"],
        )
    s = Series(
        lr(df[x], df[y]), index=["slope", "intercept", "rvalue", "pvalue", "stderr"]
    )
    s["r2"] = s["rvalue"] ** 2
    return s
...
def mask_out_outliers_by_interquartile_range(soh_values: Series) -> Series:
    ...
    q1, q3 = (
        valid_float_soh_values.quantile(0.05),
        valid_float_soh_values.quantile(0.95),
    )
...
def force_monotonic_decrease(values: Series) -> Series:
    ...
    constraints = [
        {"type": "ineq", "fun": lambda x, i=i: (x[i - 1] - x[i]) * 0.00001}
        for i in range(1, nb_vals)
    ]
    constraints += [
        {"type": "ineq", "fun": lambda x, i=i: 0.00005 * x[i - 1] - (x[i - 1] - x[i])}
        for i in range(1, nb_vals)
    ]
    ...
    results = minimize(
        objectif, notna_values, constraints=constraints, bounds=bounds, method="SLSQP"
    )
...
def evaluate_soh_estimations(results: DF, soh_cols: list[str]) -> DF:
    with warnings.catch_warnings():
        ...
        return pd.concat(
            {
                soh_col: evaluate_single_soh_estimation(results, soh_col)
...
def evaluate_single_soh_estimation(results: DF, soh_col: DF) -> DF:
    ...
    lr_params: DF = results.groupby("vin", observed=True, as_index=False).apply(
        lr_params_as_series, "odometer", soh_col, include_groups=False
    )
    return pd.Series(
        {
            "std_soh_diff_to_lr": results.merge(lr_params, "left", "vin")
            .eval(f"intercept + odometer * slope - {soh_col}")
            .std(),
            "MAE_to_aviloo_soh_readouts": results.groupby("vin", observed=True)
            .agg({soh_col: "median"})
            .merge(aviloo_readouts, "left", "vin")
...
def weighted_mean(values, weights):
    mask = weights.notna() & values.notna() & (weights < 100)
    if not mask.any():
        return np.nan
