...
from .constant_variables import DEFAULT_DIFF_VARS

def self_discharge_df_of(vehicle_df: DF, stock_kwh_per_soc: float) -> DF:
    self_discharge_df = agg_diffs_df_of(
        vehicle_df,
        "in_self_discharge_perf_mask",
        "in_self_discharge_perf_idx",
        {"cum_energy_spent": "energy_diff"}
    )
...
def motion_perfs_df_of(vehicle_df: DF, stock_kwh_per_soc: float) -> DF:
    motion_perfs_df = agg_diffs_df_of(
        vehicle_df,
        "in_motion_perf_mask",
        "in_motion_perf_idx",
        {"cum_energy_spent": "energy_diff", "odometer": "distance"}
    )
    motion_perfs_df = compute_soh_from_soc_and_energy_diff(motion_perfs_df, "energy_diff", stock_kwh_per_soc)
    motion_perfs_df["dist_per_soc"] = motion_perfs_df["distance"] / motion_perfs_df["soc_diff"]

    return motion_perfs_df

def compute_soh_from_soc_and_energy_diff(perf_df: DF, kwh_energy_col: str, stock_kwh_per_soc, soh_col="soh") -> DF:
    """
    ### Description:
    Computes the soh estimation based on a kWh diff, soc diff and stock kwh per soc ratio.
    Make sure the energy diff is in kwh.
    """
    perf_df["kwh_per_soc"] = perf_df[kwh_energy_col] / perf_df["soc_diff"]
    perf_df[soh_col] = 100 * perf_df["kwh_per_soc"] / stock_kwh_per_soc
...
def agg_diffs_df_of(vehicle_df: DF, query_str:str, groupby, get_start_end_diff_args: dict[str, str]) -> DF:
    ...
    for source_col, dest_col in {**get_start_end_diff_args, **DEFAULT_DIFF_VARS}.items():
    ...
    diffs_df["mean_date"] = diffs_df["start_date"] + diffs_df["duration"] / 2
    diffs_df["soc_diff"] = diffs_df["soc_diff"].abs()
    diffs_df["size"] = perfs_grps.size()
    diffs_df = diffs_df.query("size > 1")
