...
premium_router_hidden = APIRouter(include_in_schema=False)
...
async def get_premium_data(
    ...
    return await report_mutualized_endpoints.get_report_data(
        db, vin, user, report_type=ReportType.premium
    )
...
async def get_premium_report_for_date(
    ...
    return await report_mutualized_endpoints.get_report_for_date(
        vin, iso_date, db, s3_client, user, report_type=ReportType.premium
    )


@premium_router_hidden.get(
    "/{vin}/report_html",
...
async def get_premium_report_html_endpoint(
    ...
    return await report_mutualized_endpoints.generate_html_report(
        vin, db, user, report_type=ReportType.premium
    )


@premium_router_hidden.post(
...
async def generate_premium_report_sync_endpoint(
    ...
    return await report_mutualized_endpoints.genereate_pdf_report_sync(
        vin, db, s3_client, user, report_type=ReportType.premium
    )
