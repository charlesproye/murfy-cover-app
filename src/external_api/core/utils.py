...
def replace_closest_value(value: float, values: list[float]) -> list[float]:
    """
    Replace the closest value in the list to the value.
    """
    closest = 0
    for i, arr_value in enumerate(values):
        if abs(arr_value - value) < abs(values[closest] - value):
            closest = i
