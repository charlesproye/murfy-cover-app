...
class TeslaFTProcessedTsToRawResults(ProcessedTsToRawResults):
    ...
    def __init__(self, make='tesla-fleet-telemetry', spark: SparkSession=None, force_update:bool=False, logger: Logger=None, **kwargs):
    ...
    def compute_charge_levels(self, df):
        ...
        df = df.withColumn(
            "level_1", F.col("soc_diff") * F.when(F.col("charging_power") < LEVEL_1_MAX_POWER, 1).otherwise(0) / 100
        ).withColumn(
            "level_2", F.col("soc_diff") * F.when(F.col("charging_power").between(LEVEL_1_MAX_POWER, LEVEL_2_MAX_POWER), 1).otherwise(0) / 100
        ).withColumn(
            "level_3", F.col("soc_diff") * F.when(F.col("charging_power") > LEVEL_2_MAX_POWER, 1).otherwise(0) / 100
        )
