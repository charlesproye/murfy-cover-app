...
class RenaultProcessedTsToRawResults(ProcessedTsToRawResults):
    ...
    def __init__(self, make='renault', spark: SparkSession=None, force_update:bool=False, logger: Logger=None, **kwargs):
    ...
    def compute_specific_features(self, df):
        df = (
            df
            .filter(F.col("charging_status") == 'charging')
            .withColumn(
    ...
    def compute_soh(self, df):
        df = df.withColumn(
            "soh", 
            F.when(
                F.col("expected_battery_energy").isNotNull() & (F.col("expected_battery_energy") != 0),
    ...
    def compute_charge_levels(self, df):
        ...
        return (
            df
            .withColumn("soc_diff", F.col("soc") - F.lag("soc").over(w))
            .withColumn("level_1", F.col("soc_diff") * (F.col("charging_rate") < F.lit(LEVEL_1_MAX_POWER)).cast("int"))
            .withColumn("level_2", F.col("soc_diff") * F.col("charging_rate").between(F.lit(LEVEL_1_MAX_POWER), F.lit(LEVEL_2_MAX_POWER)).cast("int"))
            .withColumn("level_3", F.col("soc_diff") * (F.col("charging_rate") > F.lit(LEVEL_2_MAX_POWER)).cast("int"))
        )
