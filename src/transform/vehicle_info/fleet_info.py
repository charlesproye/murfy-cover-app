...
def get_google_sheet_data(
    max_retries=MAX_RETRIES, initial_delay=INITIAL_RETRY_DELAY
) -> list[dict]:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if e.response.status_code in [429, 500, 503]:  # Rate limit or server error
                ...
                logger.warning(
                    f"API error (status {e.response.status_code}), attempt {attempt + 1}/{max_retries}. "
            ...
            if attempt < max_retries - 1:
                sleep_time = min(delay * (2**attempt), MAX_RETRY_DELAY)
                logger.warning(
                    f"Unexpected error: {e!s}, attempt {attempt + 1}/{max_retries}. "
                    f"Waiting {sleep_time} seconds before retry..."
                )
                time.sleep(sleep_time)
                continue
            logger.error(
                f"Failed to fetch data after {max_retries} attempts. Last error: {last_error!s}"
            )
            raise
...
def map_col_to_dict(
    df: pd.DataFrame, col: str, mapping: dict[str, str]
) -> pd.DataFrame:
    ...
    if col in df.columns:
        df[col] = (
            df[col].str.lower().map(lambda x: mapping.get(x, x) if pd.notna(x) else x)
        )
    return df
...
def clean_version(df, model_col="model", version_col="type"):
    ...
    return df.assign(
        **{
            version_col: lambda df: df.apply(
                lambda row: row[version_col].replace(row[model_col], "")
                if pd.notna(row[model_col])
                and pd.notna(row[version_col])
                and row[model_col] in row[version_col]
                else row[version_col],
                axis=1,
            )
        }
    )
...
def format_licence_plate(df, licence_plate_col="licence_plate"):
    ...
    return df.assign(
        **{
            licence_plate_col: lambda df: df[licence_plate_col].apply(
                lambda plate: re.sub(
                    r"([a-zA-Z]+)(\d{3})([a-zA-Z]+)", r"\1-\2-\3", plate
                )
                if pd.notna(plate)
                else plate
            )
...
def standardize_model_type(
    df: pd.DataFrame, oem_col="oem", model_col="model", type_col="type"
) -> pd.DataFrame:
        ...
        if make in mappings and model in mappings[make]:
            ...
            for pattern, replacement in model_info["patterns"]:
                if re.search(pattern, type_val):
                    return pd.Series(
                        {model_col: model, type_col: str(replacement).lower()}
                    )
...
async def read_fleet_info(owner_filter: str | None = None) -> pd.DataFrame:
    ...
    try:
        ...
        # Clean column names
        df.columns = [
            col if col == "EValue" else col.lower().strip().replace(" ", "_")
        ...
        if owner_col is None:
            raise ValueError(
                f"Could not find owner column. Available columns: {df.columns.tolist()}"
            )
        ...
        if owner_filter:
            ...
            logger.info(
                f"Found {len(df)} vehicles for owner {owner_filter} (filtered from {initial_count})"
            )
        ...
        if "oem" in df.columns:
            df["oem"] = df["oem"].apply(
                lambda x: OEM_MAPPING.get(x, x.lower()) if pd.notna(x) else x
            )

        df[["oem", "make", "model", "type"]] = df[
            ["oem", "make", "model", "type"]
        ...
        for col in date_columns:
            if col in df.columns:
                df[col] = pd.to_datetime(
                    df[col], yearfirst=True, errors="coerce"
                ).dt.strftime("%Y-%m-%d")
        ...
        logger.error(f"Error reading fleet info: {e!s}")
        raise
