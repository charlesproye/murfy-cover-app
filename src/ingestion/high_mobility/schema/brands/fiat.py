...
class FiatDiagnostics(msgspec.Struct):
...
class FiatCharging(msgspec.Struct):
...
class FiatInfo(HMApiResponse):
    diagnostics: Optional[FiatDiagnostics] = None
    charging: Optional[FiatCharging] = None
    brand: str
    vin: str
    request_id: str


class MergedFiatDiagnostics(msgspec.Struct):
    odometer: list[HMApiValue[DataWithUnit[float]]] = []

    @classmethod
    def from_initial(cls, initial: Optional[FiatDiagnostics]) -> Self:
        ret = cls()
        if initial is None:
            return ret
...
class MergedFiatCharging(msgspec.Struct):
    ...
    @classmethod
    def from_initial(cls, initial: Optional[FiatCharging]) -> Self:
        ret = cls()
    ...
    def merge(self, other: Optional[FiatCharging]):
        if other is not None:
...
@register_merged
class MergedFiatInfo(msgspec.Struct):
    diagnostics: MergedFiatDiagnostics
    charging: MergedFiatCharging
    brand: str
    vin: str
    request_id: str
    ...
    @classmethod
    def from_initial(cls, initial: FiatInfo) -> Self:
        return cls(
            MergedFiatDiagnostics.from_initial(initial.diagnostics),
            MergedFiatCharging.from_initial(initial.charging),
            brand=initial.brand,
            vin=initial.vin,
            request_id=initial.request_id
        )

    def merge(self, other: FiatInfo):
