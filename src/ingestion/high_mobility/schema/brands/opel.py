...
class OpelDiagnostics(msgspec.Struct):
...
class OpelCharging(msgspec.Struct):
...
class OpelInfo(HMApiResponse):
    diagnostics: Optional[OpelDiagnostics] = None
    charging: Optional[OpelCharging] = None
    brand: str
    vin: str
    request_id: str


class MergedOpelDiagnostics(msgspec.Struct):
    odometer: list[HMApiValue[DataWithUnit[float]]] = []

    @classmethod
    def from_initial(cls, initial: Optional[OpelDiagnostics]) -> Self:
        ret = cls()
        if initial is None:
            return ret
...
@register_merged
class MergedOpelInfo(msgspec.Struct):
    diagnostics: MergedOpelDiagnostics
    charging: MergedOpelCharging
    brand: str
    vin: str
    request_id: str
    ...
    @classmethod
    def from_initial(cls, initial: OpelInfo) -> Self:
        return cls(
            MergedOpelDiagnostics.from_initial(initial.diagnostics),
            MergedOpelCharging.from_initial(initial.charging),
            brand=initial.brand,
            vin=initial.vin,
            request_id=initial.request_id
        )

    def merge(self, other: OpelInfo):
