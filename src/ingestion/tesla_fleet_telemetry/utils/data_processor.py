...
def process_telemetry_data(raw_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """
    ...
    try:
        ...
        if not all(key in raw_data for key in ["vin", "createdAt", "data"]):
            logger.warning("Données de télémétrie incomplètes: manque vin, createdAt ou data")
            return None
        ...
        try:
            ...
            logger.warning(f"Erreur de parsing de la date {created_at}: {str(e)}")
            readable_date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        ...
        if not data_items or not isinstance(data_items, list):
            logger.warning(f"Données de télémétrie vides ou format invalide pour le VIN {vin}")
            return None
        ...
        for item in data_items:
            ...
            elif "invalid" in value_container and value_container["invalid"]:
                value = None
            else:
