...
async def cleanup_clients():
    ...
    if _s3_async_client is not None:
        try:
            ...
            logger.error(f"Error closing async S3 client: {e!s}")
    ...
    if _s3_sync_client is not None:
        try:
            ...
            logger.error(f"Error closing sync S3 client: {e!s}")
...
async def save_data_to_s3(data: list[dict[str, Any]], vin: str) -> bool:
    ...
    if save_tasks:
        ...
        success = all(result for result in results if not isinstance(result, Exception))
...
async def save_object_to_s3(s3_client, bucket_name: str, key: str, data: Any) -> bool:
    ...
    try:
        ...
        logger.error(f"Error saving data to S3: {e!s}")
        return False
            ...
            while retry_count <= max_retries:
                try:
                    ...
                    if (
                        ...
                        retry_count += 1
                        logger.warning(
                            f"RequestTimeTooSkewed error, retry {retry_count}/{max_retries}"
                        )
                        ...
                        if time_offset is None:
                            logger.error(
                                "Failed to sync time, retrying without time adjustment"
                            )
                        else:
...
async def save_with_retry(
    s3_client, bucket_name: str, key: str, data: Any, max_retries: int = 3
) -> bool:
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt == max_retries - 1:
                logger.error(
                    f"Failed to save to S3 after {max_retries} attempts. Key: {key}, Error: {e!s}"
                )
                raise
...
async def delete_with_retry(
    s3_client, bucket_name: str, key: str, max_retries: int = 3
) -> bool:
    """
    ...
    for attempt in range(max_retries):
        try:
            await delete_object_with_retry(s3_client, Bucket=bucket_name, Key=key)
            return True
        except ClientError as e:
            # If this is a RequestTimeTooSkewed error, let the decorator handle it
            if e.response.get("Error", {}).get("Code") == "RequestTimeTooSkewed":
            ...
            if attempt == max_retries - 1:
                logger.error(
                    f"Failed to delete from S3 after {max_retries} attempts. Key: {key}, Error: {e!s}"
                )
                raise
            await asyncio.sleep(random.uniform(0.1, 0.5) * (attempt + 1))
        except Exception as e:
        ...
        for prefix in response.get("CommonPrefixes", []):
            ...
            if vehicle_prefix:
                ...
                task = asyncio.create_task(
                    cleanup_vehicle_data(s3_client, bucket_name, vin, cutoff_date)
                )
...
async def cleanup_vehicle_data(
    s3_client, bucket_name: str, vin: str, cutoff_date: datetime
) -> bool:
    """
    ...
    try:
        ...
        response = await list_objects_v2_with_retry(
            s3_client, Bucket=bucket_name, Prefix=vehicle_prefix
        )
        ...
        for obj in response["Contents"]:
            ...
            try:
                ...
                logger.warning(f"Unable to parse date from file {key}: {e!s}")
                continue
        ...
        for i in range(0, len(files_to_delete), batch_size):
            ...
            delete_tasks = [
                delete_with_retry(s3_client, bucket_name, key) for key in batch
            ]
...
async def sync_time_with_aws() -> float | None:
    """
    ...
    try:
        ...
        logger.warning(f"Could not sync with NTP: {e!s}")
    ...
    try:
        ...
        try:
            ...
            await list_objects_v2_simple(s3_client, Bucket=bucket_name, MaxKeys=1)
            ...
            if error_code == "RequestTimeTooSkewed":
                    ...
                    if callable(obj) and obj.__name__ == "retry_on_time_skewed":
                        ...
                        for attr_name in dir(obj):
                            if attr_name.startswith("global_time_offset"):
                                ...
                                if time_offset != 0:
                                    logger.warning(
                                        f"Detected time offset with AWS: {time_offset:.2f} seconds"
                                    )
                                    return time_offset
                ...
                match = re.search(
                    r"server time is approx ([0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}Z)",
                    error_message,
                )
                if match:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"System time is {abs(time_offset):.2f} seconds {'behind' if time_offset > 0 else 'ahead of'} AWS time"
                        )
                        ...
                        if abs(time_offset) > 60:  # More than a minute
                            logger.warning(
                                "IMPORTANT: System clock is significantly skewed. Consider synchronizing "
                                "your system time using NTP: 'sudo ntpdate pool.ntp.org'"
                            )

                        return time_offset
                    except Exception as parse_err:
                        logger.error(f"Error parsing AWS time: {parse_err!s}")

                # If we still don't have an offset, use NTP as last resort
                return await force_time_sync()
            else:
                # Different error
                logger.error(f"Unexpected error checking time sync: {e!s}")
                return None
    except Exception as e:
        logger.error(f"Error checking time synchronization: {e!s}")
        return None
...
async def force_time_sync():
    ...
    try:
        ...
        for server in NTP_SERVERS:
            try:
                ...
                try:
                    ...
                    # Extract timestamp
                    t = (
                        struct.unpack("!12I", msg)[10] - 2208988800
                    ...
                    if abs(time_offset) > 31536000:  # 1 year in seconds
                        logger.warning(
                            f"Unrealistic time offset from {server}: {time_offset} seconds, ignoring"
                        )
                        continue
                    ...
                    errors.append(f"Error with {server}: {e!s}")
                    continue
                finally:
                    client.close()

            except Exception as e:
                errors.append(f"Socket error with {server}: {e!s}")
                continue
        ...
        logger.error(f"Error in NTP synchronization: {e!s}")
        return None
