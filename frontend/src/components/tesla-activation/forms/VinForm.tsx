...
import { teslaActivationSchema, TeslaActivationForm } from './schema';
...
export const VinForm = (): React.ReactElement => {
  ...
  const formControls = useForm<TeslaActivationForm>({
  ...
  const onSubmit = async (data: TeslaActivationForm) => {
    setIsLoading(true);
    try {
